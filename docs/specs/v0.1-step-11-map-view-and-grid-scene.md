...
| 0.4 | 2026-08-15 | Codex | Restored Project Specification 1.9 canonical surfaces and mandatory requirement phrasing without changing requirement intent or acceptance. |
...
| --- | --- | --- | --- | --- |
| FR-001 | The system shall provide `GridScene` as a `QGraphicsScene` subclass that accepts `width`, `height`, and `depth` constructor arguments. | One reusable scene must represent variable canonical sector dimensions. | Construction with boundary-valid dimensions exposes the exact values. | Must |
| FR-002 | The system shall reject each `GridScene` dimension that is a bool, non-int, less than 1, or above its canonical maximum with a field-specific `TypeError` or `ValueError`. | Invalid geometry must fail before partial scene population. | Parametrized tests cover every type, lower, and upper boundary with no constructed partial scene. | Must |
| FR-003 | The system shall create exactly one `QGraphicsPathItem` for each z-level, with one diamond outline per `(x, y)` cell in that level's `QPainterPath`. | Batched paths keep the empty-grid milestone compact and avoid premature cell-item behavior. | Item count equals depth; path element/geometry checks cover origin, axes, far corner, and every level. | Must |
| FR-004 | The system shall derive each diamond center and its half-tile vertices from `world_to_scene()`, `TILE_WIDTH`, and `TILE_HEIGHT` without restating the projection formula. | Step 4 remains the single geometry source of truth. | A source/behavior check and exact projected-corner tests detect duplicated or divergent formulas. | Must |
| FR-005 | The system shall use one cosmetic 1-pixel `#4A5878` pen and no fill for each level path and shall initialize a `#0A0E1A` scene background. | The canonical bright-border/deep-background pairing makes the pre-theme grid visible without line-width scaling; Step 14 may replace these provisional literals with palette tokens. | Pen width/color/cosmetic/brush/background tests and a non-empty rendered-image smoke check pass without claiming the Step 14 final theme. | Must |
| FR-006 | The system shall assign each level path a z-value that increases with world z. | Higher layers must paint above lower layers in the empty-grid stack. | Exact monotonic ordering across all configured levels passes. | Must |
| FR-007 | The system shall set an explicit padded `sceneRect` derived from the union of all level-item bounds. | Qt's auto-growing scene rect is not deterministic under rebuild/removal and camera pan needs stable bounds. | The rectangle contains every level bound plus at least half a tile of padding on each side. | Must |
| FR-008 | The system shall provide `MapView` as a `QGraphicsView` subclass that accepts a `GridScene`, uses `AnchorUnderMouse` for transformations and `AnchorViewCenter` for resizing, sets `StrongFocus`, retains or reacquires focus for handled camera input, and enables antialiasing plus smooth pixmap transforms. | These settings define the approved map camera, keyboard availability, and render quality. | Construction and interaction tests assert the scene, anchors, exact focus policy/retention, and render-hint flags. | Must |
| FR-009 | The system shall expose a read-only zoom factor derived from the current uniform transform and a `reset_zoom()` operation that restores exactly 1.0× without changing scene data. | Tests and later toolbar actions need a stable camera surface. | Zoom, reset, and scene-coordinate invariance tests pass. | Must |
| FR-010 | The system shall compute wheel zoom from vertical `QWheelEvent.angleDelta()` using `1.15 ** (delta_y / 120)` before clamping; positive delta zooms in, negative delta zooms out, handled nonzero events are accepted, and zero-y events defer to Qt's base behavior. | One exact relation supports ordinary and high-resolution deltas without swallowing unrelated scrolling. | Positive, negative, multi-step, fractional-step, and zero-y event tests prove direction and the formula. | Must |
| FR-011 | The system shall clamp wheel zoom to the closed range 0.25× through 4.0× without overshoot or transform drift. | The canonical design fixes the usable camera range. | Repeated zoom-in/out and boundary-start tests finish exactly within the range; x/y scales remain equal. | Must |
| FR-012 | The system shall preserve the scene point beneath the cursor within one scene pixel when scroll ranges allow compensation and shall keep a fully fitted scene centered on each fitted axis. | Cursor-centered zoom is useful only where scrollbars can offset the transform; fitted content has a defined centered behavior. | Tests cover an enlarged/zoomed scene with available ranges and the default fitted scene, asserting anchor tolerance only on compensable axes and center stability on fitted axes. | Must |
| FR-013 | The system shall pan through middle-button press, move, and release by adjusting view scrollbars, consuming the handled gesture, and leaving transform scale unchanged: for viewport movement `(dx, dy)`, horizontal and vertical scrollbar values change by `(-dx, -dy)` subject to Qt range clamping. | Content follows the dragged hand while preserving future left-click selection semantics that `ScrollHandDrag` would otherwise claim. | pytest-qt gesture tests at 2.0× prove exact unclamped sign/deltas, clamped boundary behavior, stable zoom, and no stuck pan state. | Must |
| FR-014 | The system shall delegate left and right mouse events through `QGraphicsView` unchanged in Step 11. | Later selection/context behavior must not be preempted by camera plumbing. | Event-spy regression tests show only middle-button gestures enter pan state. | Must |
| FR-015 | The system shall expose read-only `active_z`, initialize it to 0, and provide `set_active_z(level)` plus `change_active_z(delta)` on `GridScene`. | Z presentation needs one explicit, testable owner. | Construction and direct-method tests cover initial, interior, and boundary states. | Must |
| FR-016 | The system shall reject bool/non-int levels passed to `set_active_z()` with `TypeError` and levels outside `0 <= level < depth` with `ValueError`, without changing the prior active level. | Direct API misuse must fail atomically. | Parametrized type/range and no-mutation tests pass. | Must |
| FR-017 | The system shall require a non-bool int delta for `change_active_z()` and clamp the result to the valid level range; a boundary change beyond the range is a no-op. | Page keys must never produce an invalid level. | Positive, negative, large-delta, and repeated-boundary tests pass. | Must |
| FR-018 | The system shall set the active level path opacity to 1.0 and every other level path opacity to 0.35 after construction or an active-level change. | The player must distinguish the target layer while retaining 3D context. | Every-level transitions assert exact opacity values and no item replacement. | Must |
| FR-019 | The system shall route PageUp to `change_active_z(+1)` and PageDown to `change_active_z(-1)`, accept those keys, and delegate all other keys to the base class. | The canonical z-switch controls must work without capturing unrelated shortcuts. | Focused key tests cover both directions, both bounds, and an unrelated key. | Must |
| FR-020 | The system shall replace `mapViewPlaceholder` with one `GridScene(15, 15, 7, parent=self)` retained by a typed strong reference and one central `MapView` named `mapView`, also retained by `MainWindow`, while preserving every existing non-central widget and bridge lifecycle. | `QGraphicsView` does not own its scene; explicit QObject and Python ownership keeps the map alive and ensures window teardown owns cleanup. | Integration tests delete construction locals and force collection without losing `view.scene()`, then close/delete the window without leaked top-level widgets; all Step 10 chrome, state, bridge, and close behaviors pass. | Must |
| FR-021 | The system shall re-export `GridScene` and `MapView` from `stmrr.view.scene` in addition to the existing projection surface. | Later view modules and tests need one intentional package interface. | Exact import and `__all__` tests pass without removing prior exports. | Must |

### 7.2 Non-Functional Requirements

| ID | Category | Requirement | Measurement / Acceptance Criteria | Priority |
| --- | --- | --- | --- | --- |
| NFR-001 | Architecture | The system shall preserve all five import-linter contracts and add no model-event or Blinker import under `stmrr.view` in Step 11. | `uv run lint-imports` reports all contracts kept; source inspection finds no prohibited import. | Must |
| NFR-002 | Performance | The system shall qualify the master NFR-003 subset in Step 11: handled zoom/pan/z input responds within 16 ms and maximum-grid static repaint throughput is at least 30 FPS on the documented project workstation. | A recorded, non-CI acceptance probe names CPU/GPU/display/backend, uses 10 warm-up operations and 100 measured repetitions per handler and repaint case, reports median/p95/max, has every measured handler below 16 ms, and averages at most 33.3 ms per maximum-grid repaint. Startup, sector-load, combat-init, galaxy-render, save/load, turn, AI, animation, and long-session budgets remain owned by later master sub-specs. | Must |
| NFR-003 | Reliability | The system shall preserve finite uniform transforms, valid active-z state, and the original level-item identities through repeated zoom, pan, and z-switch operations. | A deterministic mixed-operation regression test completes 1,000 operations with all invariants intact. | Must |
| NFR-004 | Testability | The system shall support verification of all Qt behavior with pytest-qt under the configured offscreen platform without a real window manager. | Focused Step 11 tests pass with `QT_QPA_PLATFORM=offscreen`; no assertion depends on WM state. | Must |
| NFR-005 | Quality | The system shall pass Ruff, BasedPyright strict, branch-aware coverage, import-linter, and the repository aggregate coverage floor for all new and changed files. | `uv run python scripts/check.py` exits 0. | Must |

### 7.3 Interface Requirements

| ID | Interface | Requirement | Contract / Format | Acceptance Criteria |
| --- | --- | --- | --- | --- |
| IR-001 | `GridScene` constructor | The system shall accept three positional-or-keyword integers through the public constructor: `width`, `height`, `depth`. | `GridScene(width: int, height: int, depth: int, parent: QObject \| None = None)`. | Signature/type inspection and construction tests pass. |
| IR-002 | Active-z API | The system shall expose `active_z`, `set_active_z(level)`, and `change_active_z(delta)` through the scene. | Integer world z in `[0, depth)`, with direct-set errors and delta clamping per §7.1. | API and state/opacity tests pass. |
| IR-003 | `MapView` constructor | The system shall accept a `GridScene` plus an optional widget parent through the view constructor. | `MapView(scene: GridScene, parent: QWidget \| None = None)`. | Signature/type and scene-identity tests pass. |
| IR-004 | Camera API | The system shall expose `zoom_factor` and `reset_zoom()` through the view. | Uniform finite scale in `[0.25, 4.0]`; reset target `1.0`. | Direct and event-driven camera tests pass. |
| IR-005 | User input | The system shall consume vertical wheel, middle-drag, PageUp, and PageDown according to §7.1. | Qt `QWheelEvent`, `QMouseEvent`, and `QKeyEvent` overrides. | pytest-qt interaction tests pass. |
| IR-006 | Main-window composition | The system shall replace the existing central placeholder without changing the public application-builder signature. | `MainWindow` central widget is `MapView`; private typed references retain the view and parented scene; `build_main_window()` remains unchanged. | Existing app composition plus central-widget, forced-GC lifetime, and close-cleanup tests pass. |

### 7.4 Data Requirements

| ID | Data Entity | Requirement | Validation Rules | Ownership |
| --- | --- | --- | --- | --- |
| DR-001 | Grid dimensions | The system shall retain immutable-by-convention width, height, and depth values in `GridScene` for geometry construction. | Positive non-bool integers; x/y no greater than 20; depth no greater than 7. | `GridScene` |
| DR-002 | Active z | The system shall retain one in-memory active level in `GridScene`. | Integer in `[0, depth)`; default 0; no persistence in Step 11. | `GridScene` |
| DR-003 | Camera state | The system shall retain only the ordinary Qt transform and transient middle-pan state in `MapView`. | Uniform finite zoom clamped to `[0.25, 4.0]`; pan state clears on middle release or `focusOutEvent()`. | `MapView` |
| DR-004 | Level items | The system shall retain a stable ordered collection in `GridScene` that maps every z-level to its path item. | One unique item per level; identities do not change during active-z updates. | `GridScene` |
