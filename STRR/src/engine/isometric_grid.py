...
class GridRenderer:
    ...
    DEFAULT_Z_LEVEL_COLORS: Final[tuple[ColorTuple, ...]] = (
        (80, 80, 120),   # Z=0 - Deep space
        (100, 100, 140), # Z=1
        (120, 120, 160), # Z=2
        (140, 140, 180), # Z=3 - High orbit
    )
    ...
    def __init__(
        ...
        """
        self.tile_width: Final[int] = tile_width
        self.tile_height: Final[int] = tile_height
        self.grid_width: Final[int] = grid_width
        self.grid_height: Final[int] = grid_height
        self.max_z_levels: Final[int] = max_z_levels

        # Camera and viewport
        self.camera_offset: list[int] = list(camera_offset)
        ...
        logger.info(
            ...
            grid_width, grid_height, max_z_levels
        )
    ...
    def world_to_screen(self, position: GridPosition) -> ScreenCoord:
        ...
        # Apply camera offset
        screen_x = iso_x + self.camera_offset[0]
        screen_y = iso_y + self.camera_offset[1]
    ...
    def screen_to_world(
        ...
        screen_x = screen_pos[0] - self.camera_offset[0]
        screen_y = screen_pos[1] - self.camera_offset[1]
        ...
        world_x = (screen_x / (self.tile_width / 2) +
                   screen_y / (self.tile_height / 2)) / 2
        world_y = (screen_y / (self.tile_height / 2) -
                   screen_x / (self.tile_width / 2)) / 2
    ...
    def render_z_level(self, surface: pygame.Surface, z_level: int) -> None:
        ...
        # Get color for this z-level
        color_index = min(z_level, len(self.z_level_colors) - 1)
        level_color = self.z_level_colors[color_index]

        # Draw grid lines
        self._draw_grid_lines(surface, z_level, level_color)
    ...
    def is_in_bounds(self, position: GridPosition) -> bool:
        ...
        return (0 <= position.x < self.grid_width and
                0 <= position.y < self.grid_height and
                0 <= position.z < self.max_z_levels)
    ...
    def _draw_grid_lines(
        ...
        """
        Draw grid lines for a specific z-level.
